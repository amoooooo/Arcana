...
public abstract class NodeType{
	...
	public void tick(Level world, AuraView nodes, Node node){
		...
		if(node.timeUntilRecharge <= 0){
			...
			if(node.getAspects().getHolders().stream().mapToDouble(h -> h.getStack().getAmount()).sum() < rechargeCap(world, nodes, node)){
				...
				while(toCharge > 0){
					AspectHolder holder = node.getAspects().getHolders().get(world.getRandom().nextInt(node.getAspects().getHolders().size()));
